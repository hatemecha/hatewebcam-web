...
function collectMethodNames() {
  ...
  for (const file of readdirSync(appDir)) {
    if (!file.endsWith('.mjs') || file === 'controller.mjs' || file === 'constants.mjs') continue;
    const content = readFileSync(resolve(appDir, file), 'utf8');
    for (const match of content.matchAll(/proto\.(\w+) = /g)) names.add(match[1]);
  }
...
const GLOBAL_IDENTIFIERS = new Set([
  'console', 'document', 'window', 'navigator', 'performance', 'Math', 'JSON', 'URL',
  'Date', 'Promise', 'setTimeout', 'clearTimeout', 'setInterval', 'clearInterval',
  'requestAnimationFrame', 'cancelAnimationFrame', 'parseInt', 'parseFloat', 'Number',
  'String', 'Array', 'Object', 'Error', 'Blob', 'File', 'FileReader', 'MediaRecorder',
  'VideoEncoder', 'VideoFrame', 'FaceMesh', 'CameraManager', 'EffectManager',
  'BlobTracking', 'FaceDetection', 'BlinkDetection', 'VideoTimeline', 'EditorHistory',
  'Int32Array', 'Uint8Array', 'Uint8ClampedArray', 'WeakMap', 'Map', 'Set', 'RegExp',
  'ImageData', 'HTMLCanvasElement', 'HTMLVideoElement', 'Event', 'KeyboardEvent',
  'PointerEvent', 'MouseEvent', 'ResizeObserver', 'IntersectionObserver', 'alert',
  'prompt', 'confirm', 'fetch', 'import', 'proto', 'app', 'clamp', 'escapeHtml',
]);
...
function fixSpreadState(code) {
  ...
  for (const name of STATE_VARS) {
    out = out.replace(new RegExp(`\\.\\.\\.${name}\\b`, 'g'), `...this.${name}`);
  }
  return out;
}
...
function fixMethodCalls(code) {
  ...
  for (const name of METHOD_NAMES) {
    ...
    const re = new RegExp(`(?<!\\bthis\\.|\\bproto\\.|\\bfunction |\\basync )\\b${name}\\(`, 'g');
...
function splitModalFocus(content) {
  ...
  const head = content.slice(0, idx).replace(/^\s*this\.modalFocusState = new WeakMap\(\);\s*\n/m, '');
...
writeFileSync(capturePath, fixMethodCalls(fixArgColons(fixSpreadState(
  readFileSync(capturePath, 'utf8') + captureTail.replace(/\n\}\n$/, '\n')
))));
