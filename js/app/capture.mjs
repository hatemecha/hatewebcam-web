...
export function applyCaptureMixin(proto) {
  proto.isCapturePreviewOpen = function () {
    return !!this.capturePreviewModal && !this.capturePreviewModal.classList.contains('hidden');
  }
  ...
  proto.updatePreviewPhotoEnhancerControls = function () {
    if (!this.previewPhotoEnhancerStrengthGroup || !this.sldPreviewPhotoEnhancerStrength) return;
    const enabled = this.isPendingPhotoCapture() && !!this.pendingCapture.previewEnhancerEnabled;
  ...
  proto.syncPreviewPhotoTools = function () {
    ...
    if (!isPhoto) {
      if (this.chkPreviewPhotoEnhancer) this.chkPreviewPhotoEnhancer.checked = false;
      if (this.sldPreviewPhotoEnhancerStrength) {
        this.sldPreviewPhotoEnhancerStrength.value = String(this.imageSettings.qualityEnhancerStrength);
      }
    ...
    const parsedStrength = parseInt(this.pendingCapture.previewEnhancerStrength, 10);
    const strength = this.clamp(
      Number.isFinite(parsedStrength) ? parsedStrength : this.imageSettings.qualityEnhancerStrength,
      0,
      100
    );
    ...
    if (this.chkPreviewPhotoEnhancer) this.chkPreviewPhotoEnhancer.checked = enabled;
    if (this.sldPreviewPhotoEnhancerStrength) this.sldPreviewPhotoEnhancerStrength.value = String(strength);
    if (this.valPreviewPhotoEnhancerStrength) this.valPreviewPhotoEnhancerStrength.textContent = `${strength}%`;
    this.updatePreviewPhotoEnhancerControls();
  }

  proto.rebuildPendingPhotoPreview = async function () {
    if (!this.isPendingPhotoCapture() || !this.pendingCapture.baseCanvas) return;
    const token = ++this.photoPreviewRenderToken;
    const enabled = !!this.pendingCapture.previewEnhancerEnabled;
    const strength = this.clamp(parseInt(this.pendingCapture.previewEnhancerStrength, 10) || 0, 0, 100);
    ...
    try {
      ...
      if (token !== this.photoPreviewRenderToken || !this.isPendingPhotoCapture()) return;
      ...
      this.showStatus(this.captureStatus, 'No se pudo aplicar el mejorador a la foto', 'error');
    }
  }
  ...
  proto.updateCaptureCountdownUI = function () {
    ...
    this.captureCountdown.classList.toggle('hidden', !this.isPhotoCountdownActive);
    this.captureCountdown.classList.toggle('is-final', this.isPhotoCountdownActive && this.photoCountdownRemaining <= 1);
    if (this.captureCountdownValue) {
      this.captureCountdownValue.textContent = String(Math.max(1, this.photoCountdownRemaining));
    }
  }
  ...
  proto.validatePhotoCaptureReady = function () {
    if (!this.isRunning) {
      this.showStatus(this.captureStatus, 'Primero encendé la cámara', 'warning');
      return false;
    }
    if (this.isCapturePreviewOpen()) {
      this.showStatus(this.captureStatus, 'Primero cerrá la vista previa actual', 'info');
      return false;
    }
    const { sourceWidth, sourceHeight } = this.getSourceFrameDimensions();
    if (this.videoEl.readyState < 2 || sourceWidth <= 1 || sourceHeight <= 1) {
      this.showStatus(this.captureStatus, 'Esperá un momento y volvé a sacar la foto', 'info');
      return false;
    }
    return true;
  }
  ...
  proto.startPhotoCountdown = function (seconds) {
    ...
    this.photoCountdownTimer = setInterval(() => {
      ...
      this.updateCaptureCountdownUI();
      this.showStatus(this.captureStatus, `Foto en ${this.photoCountdownRemaining} segundos`, 'info');
    }, 1000);
  }
  ...
  proto.requestPhotoCapture = function () {
    ...
    const timerSeconds = this.normalizeCaptureTimerSeconds(this.imageSettings.captureTimerSeconds);
    if (timerSeconds > 0) {
  ...
  proto.updateCaptureButtons = function () {
    ...
    const lockCaptureSettings = this.isRecording || previewOpen || this.isPhotoCountdownActive;
    const lockDetectorControls = previewOpen;

    if (this.btnTakePhoto) {
      this.btnTakePhoto.disabled = (!this.isRunning || this.isRecording || previewOpen) && !this.isPhotoCountdownActive;
    ...
    if (this.btnRecord) this.btnRecord.disabled = (!this.isRunning && !this.isRecording) || previewOpen || this.isPhotoCountdownActive;
    if (this.videoFormatSelect) this.videoFormatSelect.disabled = lockCaptureSettings;
    if (this.sldJpegQuality) this.sldJpegQuality.disabled = lockCaptureSettings;
    if (this.captureTimerSelect) this.captureTimerSelect.disabled = lockCaptureSettings;
    if (this.chkQualityEnhancer) this.chkQualityEnhancer.disabled = lockCaptureSettings;
    if (this.sldQualityEnhancerStrength) {
      this.sldQualityEnhancerStrength.disabled = lockCaptureSettings || !this.imageSettings.qualityEnhancer;
    }
    ...
    if (this.btnRecord) {
      if (this.isRecording) {
        ...
        this.btnRecord.innerHTML = '<i class="fa-solid fa-circle-dot"></i> Grabar';
      }
    }

    if (this.btnMobileTakePhoto) {
      this.btnMobileTakePhoto.disabled = (!this.isRunning || this.isRecording || previewOpen) && !this.isPhotoCountdownActive;
      this.btnMobileTakePhoto.classList.toggle('is-countdown', this.isPhotoCountdownActive);
      this.btnMobileTakePhoto.innerHTML = this.isPhotoCountdownActive
        ? '<i class="fa-solid fa-xmark"></i>'
        : '<i class="fa-solid fa-camera"></i>';
    }
    if (this.selMobileCaptureTimer) this.selMobileCaptureTimer.disabled = lockCaptureSettings;
    if (this.btnMobileEffectsDock) {
      this.btnMobileEffectsDock.disabled = previewOpen;
    }
    if (this.btnMobileRecord) {
      this.btnMobileRecord.disabled = (!this.isRunning && !this.isRecording) || previewOpen || this.isPhotoCountdownActive;
    ...
    if (this.btnMobileBlobToggle) this.btnMobileBlobToggle.disabled = lockDetectorControls;
    if (this.btnMobileFaceToggle) this.btnMobileFaceToggle.disabled = lockDetectorControls;
    if (this.btnMobileBlinkToggle) this.btnMobileBlinkToggle.disabled = lockDetectorControls;
    if (this.btnMobileColorPick) this.btnMobileColorPick.disabled = !this.chkBlobTracking.checked || !this.isRunning || lockDetectorControls;
    if (this.inpMobileBlobColor) this.inpMobileBlobColor.disabled = !this.chkBlobTracking.checked || lockDetectorControls;
    if (this.inpMobileFaceColor) this.inpMobileFaceColor.disabled = !this.chkFaceDetection.checked || lockDetectorControls;
    if (this.inpMobileFaceLabel) this.inpMobileFaceLabel.disabled = !this.chkFaceDetection.checked || lockDetectorControls;
  ...
  proto.getVideoExportFps = function () {
    return this.normalizeVideoFps(this.videoSourceFps) || this.DEFAULT_CAMERA_FPS;
  }
  ...
  proto.getRecommendedVideoBitrate = function (width, height, fps) {
    ...
    const safeFps = this.clamp(this.normalizeVideoFps(fps) || this.DEFAULT_CAMERA_FPS, 1, 240);
    ...
    return Math.max(this.videoSourceAverageBitrate, this.clamp(estimate, 8000000, 80000000));
  }

  proto.drawEnhancedFrameToContext = function (targetCtx, sourceCanvas, width, height, strengthPct, forExport = false) {
    ...
    if (forExport && amount > 0.05) {
      ...
      this.applySharpenFilter(imageData, this.clamp(0.18 + amount * 0.62, 0, 1));
  ...
  proto.renderSourceFrameBuffer = function (applyQualityEnhancer = false, mode = 'recording') {
    ...
    if (applyQualityEnhancer && this.imageSettings.qualityEnhancer) {
      ...
      if (this.recordingEnhancerCtx) {
        this.recordingEnhancerCtx.clearRect(0, 0, enhancerBuffer.width, enhancerBuffer.height);
        this.recordingEnhancerCtx.drawImage(this.recordingCanvas, 0, 0, enhancerBuffer.width, enhancerBuffer.height);
  ...
  proto.blitProcessedFrameToPreview = function () {
    if (!this.recordingCanvas || this.recordingCanvas.width === 0 || this.canvas.width === 0) return;
    this.ctx.drawImage(this.recordingCanvas, 0, 0, this.canvas.width, this.canvas.height);
  }
  ...
  proto.buildPhotoBlobFromCanvas = async function (baseCanvas, enhancerEnabled, enhancerStrength) {
    if (!baseCanvas || baseCanvas.width === 0 || baseCanvas.height === 0) {
      throw new Error('photo_base_canvas_invalid');
    }

    if (!enhancerEnabled) {
      const rawBlob = await this.canvasToBlobAsync(baseCanvas, 'image/jpeg', this.imageSettings.jpegQuality / 100);
    ...
    const blob = await this.canvasToBlobAsync(exportCanvas, 'image/jpeg', this.imageSettings.jpegQuality / 100);
  ...
  proto.buildPhotoCaptureSnapshot = async function (enhancerEnabled, enhancerStrength) {
    const { sourceWidth, sourceHeight } = this.getSourceFrameDimensions();
    if (sourceWidth <= 1 || sourceHeight <= 1) {
      throw new Error('source_frame_unavailable');
    }

    const { width: captureWidth, height: captureHeight } = this.getEffectiveFrameDimensions(sourceWidth, sourceHeight);
    ...
    const {
      ...
      width,
      height,
    } = await this.buildPhotoBlobFromCanvas(baseCanvas, enhancerEnabled, enhancerStrength);
    return {
      blob,
      width,
      height,
  ...
  proto.takePhoto = async function () {
    ...
    try {
      ...
      const initialEnhancerStrength = this.imageSettings.qualityEnhancerStrength;
      const {
        blob, width, height, baseCanvas,
      } = await this.buildPhotoCaptureSnapshot(initialEnhancerEnabled, initialEnhancerStrength);
  ...
  proto.startRecording = function () {
    if (this.isPhotoCountdownActive) {
      this.showStatus(this.captureStatus, 'Cancelá el temporizador antes de grabar', 'info');
      return;
    }
    if (!this.isRunning) {
      this.showStatus(this.captureStatus, 'Primero encendé la cámara', 'warning');
    ...
    if (this.isCapturePreviewOpen()) {
      this.showStatus(this.captureStatus, 'Primero cierra la vista previa actual', 'info');
      return;
    }
    if (typeof MediaRecorder === 'undefined') {
      this.showStatus(this.captureStatus, 'Tu navegador no soporta grabación', 'error');
      return;
    }

    const recordingProfile = this.getRecordingProfile();
    if (!recordingProfile) {
      this.showStatus(this.captureStatus, 'No hay formato de video compatible', 'error');
      return;
    }
    ...
    try {
      ...
      this.ensureRecordingCanvas();
      if (this.recordingCanvas.width === 0 || this.recordingCanvas.height === 0) {
        this.showStatus(this.captureStatus, 'Esperá un momento y reintentá la grabación', 'info');
        return;
      }
      ...
      this.recordingStream = this.recordingCanvas.captureStream(this.currentRecordingFps);
      ...
      this.mediaRecorder.onstop = () => {
        ...
        this.recordingChunks = [];
        const recordedWidth = this.recordingCanvas ? this.recordingCanvas.width : this.canvas.width;
        const recordedHeight = this.recordingCanvas ? this.recordingCanvas.height : this.canvas.height;
        ...
        if (shouldSave) {
          ...
          this.showStatus(this.captureStatus, 'Video listo en vista previa', 'info');
        }
      ...
      if (recordingProfile.fallbackMessage) {
        this.showStatus(this.captureStatus, recordingProfile.fallbackMessage, 'info');
      }
      ...
      this.recordingTimer = setInterval(() => {
        ...
        this.showStatus(this.captureStatus, `Grabando ${this.formatDuration(sec)}`, 'warning');
      }, 300);

      this.updateCaptureButtons();
    } catch (err) {
      console.error('Error starting recording:', err);
      this.showStatus(this.captureStatus, 'No se pudo iniciar la grabación', 'error');
  ...
  proto.stopRecording = function (saveFile) {
    ...
    if (saveFile && this.recordingStartTs > 0) {
      this.lastRecordingDurationSec = Math.max(0, (Date.now() - this.recordingStartTs) / 1000);
    } else {
      this.lastRecordingDurationSec = 0;
    }
  ...
  proto.openCapturePreview = function (capture) {
    ...
    if (this.capturePreviewTitle) {
      this.capturePreviewTitle.innerHTML = capture.kind === 'video'
    ...
    if (this.btnDownloadCapture) {
      this.btnDownloadCapture.innerHTML = capture.kind === 'video'
        ? '<i class="fa-solid fa-download"></i> Descargar video'
        : '<i class="fa-solid fa-download"></i> Descargar foto';
    }

    if (this.capturePreviewImage) {
      this.capturePreviewImage.classList.toggle('hidden', capture.kind !== 'photo');
    ...
    if (this.capturePreviewVideo) {
      this.capturePreviewVideo.classList.toggle('hidden', capture.kind !== 'video');
    ...
    this.activateModalFocusTrap(this.capturePreviewModal, this.btnDownloadCapture);
    this.updateCaptureButtons();
  }
  ...
  proto.renderCapturePreviewInfo = function (meta, kind) {
    ...
    if (meta.durationSec != null) rows.push(['Duracion', this.formatDurationDetailed(meta.durationSec)]);
  ...
  proto.clearPendingCapture = function (silent = false) {
    ...
    if (this.btnDownloadCapture) {
      this.btnDownloadCapture.innerHTML = '<i class="fa-solid fa-download"></i> Descargar';
    }
    if (this.capturePreviewTitle) {
      this.capturePreviewTitle.innerHTML = '<i class="fa-solid fa-image"></i> Vista previa de captura';
    }
    this.syncPreviewPhotoTools();
  }

  proto.closeCapturePreview = function (showDiscardStatus) {
    if (!this.capturePreviewModal || this.capturePreviewModal.classList.contains('hidden')) return;
  ...
  proto.downloadPendingCapture = function () {
    ...
    this.showStatus(this.captureStatus, `${label} guardado: ${this.pendingCapture.filename}`, 'success');
