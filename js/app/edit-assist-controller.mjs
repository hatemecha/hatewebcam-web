...
export class EditAssistController {
  ...
  bind() {
    ...
    this.isBound = true;
    this.app.btnEditAssistAnalyze?.addEventListener('click', () => {
      void this.analyze();
    });
    this.app.btnEditAssistBeat?.addEventListener('click', () =>
      this.applyMarkers(1),
    );
    this.app.btnEditAssistEvery2?.addEventListener('click', () =>
      this.applyMarkers(2),
    );
    this.app.btnEditAssistEvery4?.addEventListener('click', () =>
      this.applyMarkers(4),
    );
    this.app.btnEditAssistClear?.addEventListener('click', () =>
      this.clearMarkers(),
    );
    this.updateUI();
  }
  ...
  updateUI() {
    ...
    if (this.app.btnEditAssistAnalyze) {
      this.app.btnEditAssistAnalyze.disabled =
        !loaded || this.isAnalyzing || this.app.isVideoExporting;
    ...
    [
      this.app.btnEditAssistBeat,
      this.app.btnEditAssistEvery2,
      this.app.btnEditAssistEvery4,
    ].forEach((button) => {
      if (button)
        button.disabled =
          !loaded || !hasBeats || this.isAnalyzing || this.app.isVideoExporting;
    });
    if (this.app.btnEditAssistClear) {
      const count =
        this.app.videoTimeline?.getMarkersBySource?.('edit-assist').length || 0;
      this.app.btnEditAssistClear.disabled =
        !loaded || !count || this.isAnalyzing || this.app.isVideoExporting;
    }
    if (!this.app.editAssistResult) return;
    if (!loaded) {
      this.app.editAssistResult.textContent =
        'Importá un video para analizar su audio.';
    } else if (this.isAnalyzing) {
      this.app.editAssistResult.textContent =
        'Analizando pulsos aproximados...';
  ...
  applyMarkers(step) {
    if (!this.result?.beats?.length || !this.app.videoTimeline?.addMarkers)
      return;
    ...
    const markers = this.result.beats
      .filter(
        (beat, index) =>
          index % step === 0 &&
          beat.time >= timeline.trimStart &&
          beat.time <= timeline.trimEnd,
      )
      .map((beat) => ({
        time: beat.time,
        kind,
  ...
  show(message, type) {
    if (this.app.editAssistStatus)
      this.app.showStatus(this.app.editAssistStatus, message, type);
  }
