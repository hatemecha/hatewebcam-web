...
export function applyColorPickMixin(proto) {
  proto.enableColorPick = function () {
    if (!this.blobTrackingEffect) {
      this.showStatus(this.colorPickStatus, 'Primero activá "Detectar objetos por color"', 'warning');
      return;
    }
    this.colorPickMode = true;
    this.canvas.classList.add('color-pick-mode');
    this.showStatus(this.colorPickStatus, 'Hacé click en el video para elegir un color', 'info');
  }
  ...
  proto.onCanvasClick = function (e) {
    ...
    if (x >= 0 && x < this.canvas.width && y >= 0 && y < this.canvas.height) {
      ...
      const pixels = this.ctx.getImageData(sampleX, sampleY, sampleWidth, sampleHeight).data;
      const color = [0, 0, 0];
      ...
      this.blobTrackingEffect.setColorFromPixel(...color.map(value => Math.round(value / pixelCount)));
      this.colorPickMode = false;
      this.canvas.classList.remove('color-pick-mode');
      this.showStatus(this.colorPickStatus, 'Color seleccionado. Ya podés mover el objeto.', 'success');
