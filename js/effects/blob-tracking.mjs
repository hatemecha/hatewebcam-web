...
export class BlobTracking {
  ...
  _stabilizeBlobs(blobs, now = performance.now()) {
    ...
    for (let i = 0; i < this._trackedBlobs.length; i++) {
      ...
      if (
        !usedPrevious.has(i) &&
        now - previous.lastSeen <= this.detectionHoldMs
      )
        next.push(previous);
    }
  ...
  setConfig(config) {
    ...
    if (config.erodeIterations != null)
      this.erodeIterations = config.erodeIterations;
    if (config.dilateIterations != null)
      this.dilateIterations = config.dilateIterations;
    if (config.boxColor) this.boxColor = config.boxColor;
    if (config.boxThickness != null) this.boxThickness = config.boxThickness;
    if (config.showCoordinates != null)
